...
class UdpBeaconEngine(
    ...
    private val onBeaconReceived: (meshName: String, publicKey: String, deviceName: String, ip: String, port: Int) -> Unit
) {
    ...
    fun start(meshName: String, publicKey: String, deviceName: String, tcpPort: Int) {
        if (isRunning) return
        isRunning = true
                    ...
                    try {
                        ...
                        if (json.optString("magic") == "RIN_BEACON") {
                            ...
                            if (rKey.isNotBlank() && rKey != publicKey) {
                                onBeaconReceived(rMesh, rKey, rName, senderIp, rPort)
                            }
                        }
                    } catch (e: Exception) {
